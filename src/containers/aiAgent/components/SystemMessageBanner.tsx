...
import type { Message, MessageMetadata } from '@/types/feathers';
...
export function SystemMessageBanner({ message }: { message: Message }) {
    ...
    if (isRepair) {
        ...
        const cfg = isRepairSuccess
            ? { bg: 'bg-green-50 border-green-200', text: 'text-green-700', icon: <CheckCheck className="w-3 h-3 shrink-0" /> }
            : isRepairError
            ? { bg: 'bg-red-50 border-red-200', text: 'text-red-700', icon: <XCircle className="w-3 h-3 shrink-0" /> }
            : { bg: 'bg-amber-50 border-amber-200', text: 'text-amber-700', icon: <Wrench className="w-3 h-3 shrink-0 animate-pulse" /> };

        return (
            <div className={`mx-1 rounded-lg border ${cfg.bg} overflow-hidden`}>
                <div className={`flex items-center gap-2 px-3 py-2 text-[11px] font-medium ${cfg.text}`}>
                    {cfg.icon}
                    <span className="flex-1">
                        {isRepairStart && `Auto-repair started${attemptLabel}`}
                        {isRepairProgress && `Repairing${attemptLabel}…`}
                        {isRepairSuccess && `Repaired${durationSec ? ` in ${durationSec}s` : ''}${attemptLabel}`}
                        {isRepairError && `Auto-repair failed${attemptLabel}`}
                    </span>
                </div>

                {hasFileDetails && (isRepairSuccess || isRepairProgress) && (
                    <div className="px-3 pb-2 border-t border-current border-opacity-10 pt-1.5 divide-y divide-current divide-opacity-5">
                        {modifiedFiles.map((f) => (
                ...
                {isRepairSuccess && message.metadata?.fileDiffs && (message.metadata.fileDiffs as MessageMetadata['fileDiffs'])!.length > 0 && (
                    <div className="px-3 pb-2 pt-1 space-y-1">
                        {(message.metadata.fileDiffs as MessageMetadata['fileDiffs'])!.map(d =>
                            <FileDiff key={d.path} diff={d} />
                        )}
                    </div>
                )}
    ...
    return (
        <div className="text-center text-[10px] text-zinc-400 italic py-1 px-4">
            {message.content}
        </div>
    );
}
