...
import { CheckCircle2, ChevronDown, Clock, Copy, Loader2, Play, Plus, Save, Trash2, XCircle } from 'lucide-react';
...
export function TestPanel({ projectId, sessionId, sessionProxyUrl, isSessionRunning }: TestPanelProps) {
    ...
    const handleSendRequest = async () => {
        ...
        setRequests((prev) => prev.map((req) => (req.id === selectedRequest.id ? { ...req, status: 'loading' as const } : req)));
        ...
        try {
            ...
            setRequests((prev) => prev.map((req) => (req.id === selectedRequest.id ? { ...req, status: 'error' as const, responseTime } : req)));
        ...
        <div className="h-full flex bg-white">
            ...
            <div className="w-64 border-r border-gray-200 flex flex-col bg-gray-50">
                <div className="p-3 border-b border-gray-200 bg-white">
                    {projectId && <p className="text-[10px] text-gray-500 mb-2 truncate">Project: {projectId}</p>}
                    <Button onClick={handleAddRequest} className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm h-9">
                        <Plus className="w-4 h-4 mr-1.5" />
                        New Request
                    </Button>
                </div>
                <div className="flex-1 overflow-y-auto p-2">
                    <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide px-2 py-2">Requests</div>
                    {requests.map((req) => (
                        <button
                            key={req.id}
                            onClick={() => setSelectedRequest(req)}
                            className={`w-full text-left p-2.5 rounded-lg mb-1 transition-colors group ${
                                selectedRequest.id === req.id ? 'bg-white border border-blue-200 shadow-sm' : 'hover:bg-white/50'
                            }`}
                        >
                            <div className="flex items-center gap-2 mb-1.5">
                                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${methodColors[req.method]}`}>{req.method}</span>
                            ...
                            {req.status !== 'idle' && (
                                <div className="flex items-center gap-1.5">
                                    ...
                                    {req.responseTime && <span className="text-[10px] text-gray-500">{req.responseTime}ms</span>}
                                </div>
                            )}
                        </button>
                    ))}
                    ...
                    <div className="p-4 space-y-3">
                        ...
                        <div className="flex gap-2">
                            <div className="relative">
                                ...
                                {showMethodDropdown && (
                                    <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg overflow-hidden z-10 shadow-xl min-w-30">
                                        {methods.map((method) => (
                                            <button
                                                key={method}
                                                onClick={() => {
                            ...
                            <Button onClick={handleSaveRequest} variant="outline" className="h-10 px-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {response && (
                                <div className="flex items-center gap-3 text-xs">
                                    ...
                                    >
                                        {response.status >= 200 && response.status < 300 ? <CheckCircle2 className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
