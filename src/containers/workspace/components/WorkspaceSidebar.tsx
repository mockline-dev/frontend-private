            ...
            <div className="border-b border-border flex">
                {SIDEBAR_VIEWS.map(({ id, label }) => (
                    <button
                        ...
                        className={`flex-1 px-1 py-2 text-[11px] font-medium transition-colors flex items-center justify-center gap-1.5 ${
                            sidebarView === id
                                ? 'text-zinc-900 border-b border-zinc-900'
                                : 'text-zinc-400 hover:text-zinc-600 border-b border-transparent'
                        }`}
                    >
                        {id === 'files' && <FolderTree className="w-3 h-3" />}
                        {id === 'ai' && <Bot className="w-3 h-3" />}
                        {id === 'versions' && <History className="w-3 h-3" />}
                        {label}
                    </button>
                ))}
            </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-2 py-2">
                            {fileTree.length > 0 ? (
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-xs text-zinc-700 mt-1">Use Mocky to generate code</p>
                                </div>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="p-3 border-b border-zinc-200">
                ...
                >
                    {isSnapshotCreating ? <Loader2 className="w-3 h-3 mr-1.5 animate-spin" /> : <History className="w-3 h-3 mr-1.5" />}
                    Create Snapshot
                </Button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {snapshotsLoading ? (
                    <div className="text-[11px] text-zinc-400 px-1">Loading snapshots…</div>
                ) : snapshots.length === 0 ? (
                        ...
                        return (
                            <div key={snapshot._id} className="border border-zinc-200 rounded-lg p-2.5 bg-white">
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-500 uppercase shrink-0">
                                        {snapshot.trigger}
                                    </span>
                                </div>
